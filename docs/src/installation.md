...
[EquationsOfStateOfSolidsFitting](https://github.com/MineralsCloud/EquationsOfStateOfSolidsFitting.jl).
...
## Install EquationsOfStateOfSolidsFitting
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("EquationsOfStateOfSolidsFitting")
   ```

3. Run

   ```julia-repl
   julia> using EquationsOfStateOfSolidsFitting
   ```
...
version of EquationsOfStateOfSolidsFitting, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/EquationsOfStateOfSolidsFitting.jl"
```

in the second step above.

## Update EquationsOfStateOfSolidsFitting
...
our [GitHub repository](https://github.com/MineralsCloud/EquationsOfStateOfSolidsFitting.jl)
for new releases.
Once we release a new version, you can update EquationsOfStateOfSolidsFitting by typing

```@repl
using Pkg
Pkg.update("EquationsOfStateOfSolidsFitting")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall EquationsOfStateOfSolidsFitting
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("EquationsOfStateOfSolidsFitting")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall EquationsOfStateOfSolidsFitting.
